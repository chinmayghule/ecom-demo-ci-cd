    ...
    <header className="sticky top-0 z-50 w-full px-4 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <nav className="container flex h-16 items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <ProfileLink isActive={isActive} />

          <WishlistLink isActive={isActive} />

          <ShoppingCartLink isActive={isActive} />
        </div>
      </nav>
    </header>
  );
...
function ProfileLink({ isActive }: { isActive: (path: string) => boolean }) {
  return (
    <Link href="/profile">
      <Button
...
function WishlistLink({ isActive }: { isActive: (path: string) => boolean }) {
  return (
    <Link href="/wishlist" className="relative">
      <Button variant={isActive("/wishlist") ? "default" : "ghost"} size="sm">
